...
<header>
  <h1><button onclick={() => board = null}>{title}</button></h1>
  <div class="controls">
    <button onclick={() => mySettings.current.fetchDefinitions = !mySettings.current.fetchDefinitions} class="abs" class:faded={!mySettings.current.fetchDefinitions}>
